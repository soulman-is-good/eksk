...
class SmppClient
{
	...
	protected function _bind($login, $pass, $command_id)
	{
		...
		if (!is_string($response) && $response->status != SMPP::ESME_ROK) throw new SmppException(SMPP::getStatusMessage($response->status), $response->status);
	...
	protected function sendCommand($id, $pduBody)
	{
		...
		if ($response === false) throw new SmppException('Failed to read reply to command: 0x'.dechex($id),69);
	...
	protected function readPDU_resp($seq_number, $command_id)
	{
                if(!$this->transport->isOpen())
                    throw new RuntimeException('Socket isn\'t open.');
		...
		do{
			$pdu=$this->readPDU();
			if (true === ($inst = ($pdu instanceof SmppPdu))) {
				...
				if ($pdu->sequence == null && $pdu->id == SMPP::GENERIC_NACK) return $pdu;
                                if($this->debug){
                                    call_user_func($this->debugHandler, "unknown PDU #$pdu->id, push to queue.");
                                }
				array_push($this->pdu_queue, $pdu); // unknown PDU push to queue
                                $pdu = null;
			}else if($this->debug && $pdu === 0)
                                call_user_func($this->debugHandler, "Could not read PDU length, exiting...");
			else if($this->debug && $pdu === 1)
                                call_user_func($this->debugHandler, "Could not read PDU header, exiting...");
                        else if(is_string($pdu))
                            return $pdu;
		} while($inst);
		return false;
	}
	...
	protected function readPDU()
	{
		...
		if($bufLength === false) return 0;
                extract(unpack("Nlength", $bufLength));
		// Read PDU headers
		$bufHeaders = $this->transport->read(4);
		if($bufHeaders === false) return 1;
		extract(unpack("Ncommand_id", $bufHeaders));
		$bufHeaders = $this->transport->read(4);
		if($bufHeaders === false) return 1;
		extract(unpack("Ncommand_status", $bufHeaders));
		$bufHeaders = $this->transport->read(4);
		if($bufHeaders === false) return 1;
		extract(unpack("Nsequence_number", $bufHeaders));
		// Read PDU body
		if($length-16>0){
...
class SMPP
{
	...
	const ESME_FAILCREDIT = 0x00000400; // Fail credit
	const ESME_ACCDISABLED = 0x00000401; // Access disabled
	...
	public static function getStatusMessage($statuscode)
	{
		switch ($statuscode) {
			...
			case SMPP::ESME_FAILCREDIT: return 'Fail credit';
			case SMPP::ESME_ACCDISABLED: return 'Insuficion funds';
			default:

...
class SocketTransport
{
	...
	public function open()
	{
		...
		while ($it->valid()) {
			...
			if (!self::$forceIpv4 && !empty($ip6s)) { // Attempt IPv6s first
				foreach ($ip6s as $ip) {
                                        $tries  = 5;
					while(!($r = @socket_connect($socket6, $ip, $port)) && SOCKET_EINPROGRESS == ($error = socket_last_error()) && $tries-->0) {
                                            if ($this->debug) {
						call_user_func($this->debugHandler, socket_strerror($error));
                                            }
                                            sleep(1);
                                        }
					if ($r) {
						...
						call_user_func($this->debugHandler, "Socket connect to $ip:$port failed; ".socket_strerror($error));
					}
				}
			}
			if (!self::$forceIpv6 && !empty($ip4s)) {
				foreach ($ip4s as $ip) {
                                        $tries  = 5;
                                        while(!($r = @socket_connect($socket4, $ip, $port)) && (SOCKET_EINPROGRESS === ($error = socket_last_error()) || SOCKET_EALREADY === $error) && $tries-->0) {
                                            if ($this->debug) {
						call_user_func($this->debugHandler, socket_strerror($error));
                                            }
                                            sleep(1);
					...
					if ($r) {
						...
						call_user_func($this->debugHandler, "Socket connect to $ip:$port failed;  #$error: ".socket_strerror(socket_last_error()));
					}
				}
			}
			$it->next();
	...
	{       
                $tries = 5;
                if($this->debug) call_user_func($this->debugHandler, "Waiting for reply...");
                while ((FALSE === ($d = socket_read($this->socket,$length,PHP_BINARY_READ)) && socket_last_error() === SOCKET_EAGAIN) && $tries-->0) {
                    if($this->debug) call_user_func($this->debugHandler, "$tries...");
                    sleep(1);
                }
                if ($d === true) {
                    if($this->debug) call_user_func($this->debugHandler, "socket_read returned TRUE!");
                    $d = socket_read($this->socket,$length,PHP_BINARY_READ);
                }
		if ($d === false && socket_last_error() === SOCKET_EAGAIN){
                    if($this->debug) call_user_func($this->debugHandler, "sockets give EAGAIN on timeout");
		...
		if ($d === '') {
                    if($this->debug) call_user_func($this->debugHandler, "Returned an empty string of $length bytes.");
                    return false;
                }
		return $d;
	}

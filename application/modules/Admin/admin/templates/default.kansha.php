...
<style>
    .formtable {width:100%}
    #x3-container .treelink {text-decoration: none;color: #434f66;padding:5px;border:1px solid #434f66;border-radius:10px;}
</style>
...
<table>
<tr>
        <th align="left"><input type="checkbox" id="check-deleteall" onclick="$('[id^=\'delete-\']').each(function(){$(this).attr('checked',$('#check-deleteall').is(':checked'))})" /></th>
<?foreach ($labels as $name=>$value):
    if(isset($info[$name]['ref']) && isset($info[$name]['tree'])){
        $tree = array('name'=>$name,'query'=>$info[$name]['tree']);
    }
    if(is_array($value)){
...
foreach ($modules as $module):?>
    <tr rowid="<?=isset($pk)?$module->$pk:0?>">
        <td align="left">
            <input type="checkbox" id="delete-<?=$module->id?>" value="<?=$module->id?>" name="Delete[<?=$class?>][]" />
            <?if($tree!==false):
                $submodels = $info[$tree['name']]['ref'][0];
                $sattr = $info[$tree['name']]['ref'][1];
                $query = $tree['query'];
                $query['@condition'][$tree['name']] = $module->{$sattr};
                $treecnt = X3_Module_Table::num_rows($query, $submodels);
                if($treecnt>0):
                    $submodels = X3_Module_Table::get($query, false, $submodels);                
            ?>
            <a href="#show-tree" class="treelink">▸</a>
            <?endif;endif;?>
        </td>
<?foreach ($labels as $name=>$value):
    $v = '';
...
<?if($tree !== false && $treecnt>0):
    echo '<tr><td style="height:0px" colspan="'.(count($labels) + count($actions) + 2).'"><div style="display:none" id="tree'.$module->{$sattr}.'">' . $this->renderPartial($path,array('modules'=>$submodels,'refAttr'=>$sattr)) . '</div></td></tr>';
    ?>
